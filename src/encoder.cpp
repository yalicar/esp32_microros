...
void InitEncoder() {
    ...
    gpio_set_direction(GPIO_NUM_2, GPIO_MODE_OUTPUT); // set gpio as output
    gpio_set_level(GPIO_NUM_2, 1); // set gpio level to high
...
void encoder_speed_()
{
    ...
    encoder_speed.left = (encoder_count.left * encoder_direction.left * 60 *1000 )/ (encoder_setup.PULSES_PER_REV * encoder_setup.FRAME_TIME_MS);
...
void encoder_position_()
{
    ...
    encoder_position.x += encoder_velocity.linear * cos(encoder_position.theta) * encoder_setup.FRAME_TIME_MS/1000;
    // y position calculation (m)
    encoder_position.y += encoder_velocity.linear * sin(encoder_position.theta) * encoder_setup.FRAME_TIME_MS/1000;
    // theta position calculation (rad) frame time in ms
    encoder_position.theta += encoder_velocity.angular * encoder_setup.FRAME_TIME_MS/1000;
}
// main function
void GetEncoder()
{
